...
const PICKERTRANSITIONHEIGHT = "219px";
var Picker = {
	cellHeight: 36,
	friction: 0.003,
	slotData: [],
	...
	handleEvent: function (e) {
		if (e.type == 'touchstart') {
			this.lockScreen(e);
			if (e.currentTarget.id == 'picker-cancel' || e.currentTarget.id == 'picker-done') {
				this.tapDown(e);
			} else if (e.currentTarget.id == 'picker-frame') {
				this.scrollStart(e);
			}
	...
	create: function () {
		...
		div.innerHTML = '<toolbar>\
			<uibutton id="picker-cancel">\
					<label>Cancel</label>\
				</uibutton>\
			</uibutton>\
			<uibutton id="picker-done" ui-bar-align="right" ui-implements="done">\
				<label>Done</label>\
			</uibutton>\
			</toolbar>\
			<slotswrapper id="slots-wrapper"><slots id="sgwt-slots"></slots></slotswrapper>\
		...
		// Slots visible area:
		this.pickerSlotWrapper = document.getElementById('slots-wrapper');	
		// Pseudo table element (inner wrapper):
		this.pickerSlots = document.getElementById('sgwt-slots');
		// The scrolling controller:
		this.pickerFrame = document.getElementById('picker-frame'); 
		...
		document.getElementById('picker-cancel').addEventListener('touchstart', this, false);
		document.getElementById('picker-done').addEventListener('touchstart', this, false);
	...
	destroy: function () {
		...
		document.getElementById('picker-cancel').removeEventListener('touchstart', this, false);
		document.getElementById('picker-done').removeEventListener('touchstart', this, false);
		...
		document.body.removeChild(document.getElementById('picker-wrapper'));
	},
	...
	tapUp: function (e) {
		...
		if (e.currentTarget.id == 'picker-cancel') {
			this.cancelAction();
		} else {
			this.doneAction();
		}
